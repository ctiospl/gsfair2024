...
<script lang="ts">
	...
	import { PopupPaymentProps, ConfirmDialogProps } from '$lib/ui-item-states.svelte.js';
	...
	$effect(async () => {
		if (PopupPaymentProps.open == true) {
			pushState('?popupPayment=true', {
				popupPaymentOpened: true
			});
		}
	});
	let updateQrCode = useDebounce((e) => {
		if (isNaN(parseFloat(e.target.value))) {
			console.log(isNaN(parseFloat(e.target.value)));
			// e.target.value = 0;
			return;
		}
		PopupPaymentProps.recharge_amt = parseFloat(e.target.value);
		PopupPaymentProps.qrcode_message = `Paying ${$eventLS.event_name} - Rs ${PopupPaymentProps.recharge_amt.toFixed(2)}`;
		console.log(PopupPaymentProps.recharge_amt);
		PopupPaymentProps.qrcode_text = `${PUBLIC_UPI_URL}${PopupPaymentProps.recharge_amt}`;
	}, debounceDuration);
</script>

<Drawer.Root dismissible={false} bind:open={PopupPaymentProps.open}>
	<Drawer.Content class="!h-dvh">
		<Drawer.Header>
			<Drawer.Close
				onclick={() => {
					history.back();
					...
					// PopupPaymentProps.onConfirm = async () => {};
			...
			<Drawer.Title class="pb-4 text-center text-2xl font-semibold">
				{PopupPaymentProps.title}</Drawer.Title
			>
			<Drawer.Description class="">
				<ScrollArea class="h-[calc(100vh-10rem)]">
					<div class="space-y-2 pr-4">
						<div class="flex items-center border-b-2 border-border pb-2 text-xl font-semibold">
							<div class="">Items Total</div>
							<div class="ml-auto">₹ {PopupPaymentProps.total_amount.toFixed(2)}</div>
						</div>
						{#each PopupPaymentProps.items as item}
							<div class="flex items-center text-base">
								<div class="ml-8">{item.units} x {item.name}</div>
								<div class="ml-auto">₹ {item.amount.toFixed(2)}</div>
							</div>
						{/each}
						{#if PopupPaymentProps.use_qrcode_balance}
							<div
								class="flex items-center border-b-2 border-border pb-1 pt-4 text-xl font-semibold"
							>
								<div class="text-left">QR Code Balance</div>
								<div class="ml-auto text-destructive">
									- ₹ {PopupPaymentProps.qrcode_balance.toFixed(2)}
								</div>
							</div>
						{/if}
						<div class="flex items-center border-b-2 border-border pb-2 text-xl font-semibold">
							<div class="text-left">To collect from {PopupPaymentProps.guest_name}</div>
							<div class="ml-auto">
								<div class="relative flex items-center">
									<span class="absolute left-3 text-muted-foreground">₹</span>
									<Input
										onkeyup={updateQrCode}
										value={PopupPaymentProps.recharge_amt}
										class="w-32 text-right text-xl font-semibold"
									/>
								</div>
							</div>
						</div>
					...
					<div class="mt-6 space-y-4">
						<h2 class="text-center text-lg font-semibold">Confirm Payment Method - UPI/Cash</h2>

						<div class="flex justify-center gap-16 pb-8 pt-4">
							<Button
								variant="link"
								class="flex h-auto items-center gap-2"
								onclick={() => {
									ConfirmDialogProps.title = 'Confirm Payment Receipt (UPI)';
									ConfirmDialogProps.description = `Please Confirm the reciept of amount <b>₹ ${PopupPaymentProps.recharge_amt.toFixed(2)}</b> from Chintan (UPI)`;
									ConfirmDialogProps.confirmButtonText = 'Yes';
									ConfirmDialogProps.cancelButtonText = 'No';
									ConfirmDialogProps.onConfirm = async () => {
										ConfirmDialogProps.open = false;
										PopupPaymentProps.payment_method = 'upi';
										await PopupPaymentProps.onConfirm('upi');
									};
									ConfirmDialogProps.open = true;
									console.log(PopupPaymentProps);
								}}
							>
								GPay
								<svg
									...
									/><path fill="#e97208" d="M262762 55833l16343 32510-34395 32510z" /><path
							...
							<Button
								...
								class="flex h-auto items-center gap-2"
								onclick={() => {
									ConfirmDialogProps.title = 'Confirm Payment Receipt (Cash)';
									ConfirmDialogProps.description = `Please Confirm the reciept of amount <b>₹ ${PopupPaymentProps.recharge_amt.toFixed(2)}</b> from Chintan (Cash)`;
									ConfirmDialogProps.confirmButtonText = 'Yes';
									ConfirmDialogProps.cancelButtonText = 'No';
									ConfirmDialogProps.onConfirm = async () => {
										ConfirmDialogProps.open = false;
										PopupPaymentProps.payment_method = 'cash';
										await PopupPaymentProps.onConfirm('cash');
									};
									ConfirmDialogProps.open = true;
									console.log(PopupPaymentProps);
								}}
							>
								<svg
									...
									fill-rule="evenodd"
									clip-rule="evenodd"
									viewBox="0 0 512 296.937"
									class="!size-24"
									><path
										fill="#427D2A"
										d="M55.708 0H512v253.196l-32.271.146 2.253-185.798.086-2.218c.019-16.411-13.279-29.735-29.691-29.754l-396.669-.156V0z"
									/><path fill="#427D2A" d="M0 60.065h456.298v236.872H0z" /><path
										fill="#87CC71"
										d="M382.151 96.888c-.241 17.62 15.287 33.558 33.564 33.777v93.72c-19.252-.241-35.187 15.287-35.406 36.278H76.733c.242-19.252-15.29-35.191-33.563-35.41v-93.72c19.325.219 35.273-15.3 35.492-34.645h303.489z"
									/><path
										fill="#427D2A"
										d="M165.773 145.514c18.719-35.166 62.393-48.495 97.559-29.777 35.165 18.716 48.495 62.394 29.776 97.559-18.715 35.165-62.393 48.495-97.558 29.777-35.166-18.719-48.495-62.394-29.777-97.559z"
									/><path
										fill="#FEFEFE"
										d="M204.283 138.196h50.371c.432 0 .785.362.785.806v7.676a.797.797 0 01-.785.805h-15.732c2.48 2.885 4.252 6.36 5.055 10.179h10.677c.432 0 .785.361.785.805v7.677a.798.798 0 01-.785.806h-10.677c-.962 4.574-3.318 8.659-6.613 11.821-4.316 4.141-10.258 6.71-16.789 6.71v.026h-.726l28.254 32.218c.764.872-.574 2.811-1.173 2.813l-13.064.074-30.161-34.394a1.055 1.055 0 01-.206-1.018v-12.597l17.076.001v.024c3.07 0 5.854-1.199 7.869-3.13a10.56 10.56 0 001.958-2.548h-26.119a.797.797 0 01-.783-.806v-7.677c0-.444.353-.805.783-.805h26.119a10.516 10.516 0 00-1.958-2.547c-2.015-1.934-4.799-3.132-7.869-3.132v.024l-17.076.001v-13.006c0-.445.353-.806.784-.806z"
									/></svg
								>
								Cash
							</Button>
					...
					<div>
						{#key PopupPaymentProps.qrcode_text}
							<StyledQrCode
								class="flex items-center"
								qrcodeText={PopupPaymentProps.qrcode_text}
								message={PopupPaymentProps.qrcode_message}
							/>
						{/key}
					</div>
				</ScrollArea>
