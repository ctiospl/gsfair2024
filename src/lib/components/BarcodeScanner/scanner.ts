...
async function createBarcodeDetector(formats: BarcodeFormat[]): Promise<BarcodeDetector> {
  if (window.BarcodeDetector === undefined) {
    console.debug('BarcodeScanner Native BarcodeDetector not supported. Will use polyfill.')
  ...
  if (unsupportedFormats.length > 0) {
    console.debug(`BarcodeScanner Native BarcodeDetector does not support formats ${JSON.stringify(unsupportedFormats)}. Will use polyfill.`)
    return new BarcodeDetector({ formats })
  }

  console.debug('BarcodeScanner Will use native BarcodeDetector.')
...
export const keepScanning = async (
  videoElement: HTMLVideoElement,
  {
    ...
    try {
			console.debug('BarcodeScanner start scanning');
			...
			const processFrame =
				...
				async (timeNow: number) => {
					if (videoElement.readyState === 0) {
						console.debug('BarcodeScanner stop scanning: video element readyState is 0');
					} else {
